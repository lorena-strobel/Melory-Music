class ItemsController < ApplicationController
  ...
  def create # lógica de add_item
    ...
    if @item
      new_quantity = @item.quantity_item.to_i + item_params[:quantity_item].to_i # se existir adiciona quantidade é adicionada a existente
  ...
  def destroy
    begin
      ...
      @item.destroy!
      respond_to do |format|
          format.html { redirect_to items_path, notice: "Item foi excluído com sucesso.", status: :see_other }
  ...
  private
    ...
    def total_value
      @total = Item.find_by(item_params[:prince_item]).sum *Item.find_by(item_params[:quantity_item])
    end
end
